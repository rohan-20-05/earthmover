...
class Machine(models.Model):
    ...
    image_url = models.URLField(
        blank=True,
        null=True,
        help_text="Paste image URL from Google/Imgur"
    )
...
class Booking(models.Model):
    ...
    # UPDATED: Machine stored as text instead of ForeignKey
    ...
    def clean(self):
        """
        Prevent double booking for the same
        machine on same date/time slot.
        """
        ...
        if self.start_time and self.end_time:
            ...
            for booking in overlapping:
                ...
                if not (
                    self.end_time <= booking.start_time or
    ...
    def __str__(self):
        return f"{self.machine} | {self.date} | {self.user.name}"
